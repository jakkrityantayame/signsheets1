...
class SentDocumentsScreen extends StatefulWidget {
  @override
  _SentDocumentsScreenState createState() => _SentDocumentsScreenState();
}

class _SentDocumentsScreenState extends State<SentDocumentsScreen> {
    ...
    try {
      ...
      final convertedData = data.map((row) {
        return row.asMap().entries.map((entry) {
          ...
          if (index == 1) { // วันที่ออกเอกสาร
            return convertSerialToDate(value);
          }
          return value;
        }).toList();
      }).toList();
      // กรองเฉพาะเอกสารที่มีประเภทเป็น "ส่ง" (index 8)
      final sentData = convertedData.where((row) => row.length > 8 && row[8] == 'ส่ง').toList();
      setState(() {
        _data = sentData;
        _filteredData = sentData;
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
        ...
        children: [
          ...
          Expanded(
            child: _isLoading
                ...
                ? Center(child: Text("ไม่มีหนังสือส่ง"))
                : ListView.builder(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => AddDataScreen(isSentDocument: true),
            ),
          ).then((_) => _fetchData());
        },
